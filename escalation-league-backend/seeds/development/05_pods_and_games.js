...
exports.seed = async function (knex) {
  ...
  await knex('user_leagues').where({ user_id: frankId, league_id: league.id }).increment('league_wins', 1);
  await knex('user_leagues').where({ user_id: charlieId, league_id: league.id }).increment('league_losses', 1);
  await knex('user_leagues').where({ user_id: bobId, league_id: league.id }).increment('league_losses', 1);
